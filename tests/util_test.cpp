...
TEST(DetailRandom, RngStateAdvancesThroughWrapper) {
  ...
  walnutpie::detail::Random<std::mt19937> r(rng);
...
TEST(LogSumExp, KnownValues) {
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(0.0, 0.0), std::log(2.0), 1e-15);
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(-3.0, -3.0), -3.0 + std::log(2.0),
              1e-15);
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(1000.0, 1000.0),
              1000.0 + std::log(2.0), 1e-10);
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(1.0, 2.0),
...
TEST(LogSumExp, Symmetry) {
  EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(1.0, 2.0),
                   walnutpie::detail::log_sum_exp(2.0, 1.0));
  EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(-5.0, 3.0),
                   walnutpie::detail::log_sum_exp(3.0, -5.0));
}

TEST(LogSumExp, LargeDifferenceApproximatesMax) {
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(1000.0, 0.0), 1000.0, 1e-10);
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(0.0, 1000.0), 1000.0, 1e-10);
  EXPECT_NEAR(walnutpie::detail::log_sum_exp(-1.0, -1000.0), -1.0, 1e-10);
}

TEST(LogSumExp, NumericalStabilityLargePositive) {
  double result = walnutpie::detail::log_sum_exp(1e308, 1e308);
...
TEST(LogSumExp, InfiniteArgs) {
  ...
  double neg_inf = -inf;
  EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(neg_inf, 5.0), 5.0);
  EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(5.0, neg_inf), 5.0);
  EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(neg_inf, -3.0), -3.0);
  EXPECT_EQ(walnutpie::detail::log_sum_exp(neg_inf, neg_inf), neg_inf);

  EXPECT_EQ(walnutpie::detail::log_sum_exp(inf, 0.0), inf);
  EXPECT_EQ(walnutpie::detail::log_sum_exp(0.0, inf), inf);
  EXPECT_EQ(walnutpie::detail::log_sum_exp(inf, inf), inf);

  EXPECT_TRUE(std::isinf(walnutpie::detail::log_sum_exp(inf, neg_inf)));
  EXPECT_TRUE(std::isinf(walnutpie::detail::log_sum_exp(neg_inf, inf)));
}

TEST(LogSumExp, NaNPropagates) {
  double nan = std::numeric_limits<double>::quiet_NaN();
  EXPECT_TRUE(std::isnan(walnutpie::detail::log_sum_exp(nan, 1.0)));
  EXPECT_TRUE(std::isnan(walnutpie::detail::log_sum_exp(1.0, nan)));
  EXPECT_TRUE(std::isnan(walnutpie::detail::log_sum_exp(nan, nan)));
}

// walnutpie::detail::log_sum_exp(VectorXd) function ******************

TEST(LogSumExpVector, EmptyVector) {
  Eigen::VectorXd x(0);
  EXPECT_EQ(walnutpie::detail::log_sum_exp(x),
...
TEST(LogSumExpVector, SingleElement) {
  for (auto x : std::vector<double>{-1000.0, 0.0, 3.7, 1000.2}) {
    Eigen::VectorXd v(1);
    v << x;
    EXPECT_DOUBLE_EQ(walnutpie::detail::log_sum_exp(v), x);
  }
}
...
TEST(LogSumExpVector, EqualElements) {
  for (int n : {2, 5, 100}) {
    ...
    EXPECT_NEAR(walnutpie::detail::log_sum_exp(x), -4.0 + std::log(n), 1e-12);
  }
}
...
TEST(LogSumExpVector, StabilityLargePositive) {
  Eigen::VectorXd x(3);
  x << 1e308, 1e308, 1e308;
  double result = walnutpie::detail::log_sum_exp(x);
  EXPECT_FALSE(std::isinf(result));
...
TEST(LogSumExpVector, AllInfinity) {
  ...
  EXPECT_EQ(walnutpie::detail::log_sum_exp(x), inf);
  EXPECT_EQ(walnutpie::detail::log_sum_exp(-x), -inf);
}
...
TEST(LogSumExpVector, OnePositiveInfinity) {
  ...
  x << 1.0, -inf, 2.0;
  EXPECT_EQ(walnutpie::detail::log_sum_exp(x),
            walnutpie::detail::log_sum_exp(1.0, 2.0));
}
...
TEST(LogSumExpVector, MixedInfinities) {
  ...
  Eigen::VectorXd x(3);
  x << inf, 1.0, -inf;
  auto lse = walnutpie::detail::log_sum_exp(x);
...
TEST(LogSumExpVector, NaNPropagates) {
  ...
  Eigen::VectorXd x(3);
  x << 1.0, nan, 2.0;
  EXPECT_TRUE(std::isnan(walnutpie::detail::log_sum_exp(x)));
}

// walnutpie::detail::logp_momentum() function
// *****************************************
...
TEST(LogpMomentum, SingleElement) {
  ...
  rho << 2.0;
  inv_mass << 1.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, inv_mass), -2.0);
}
...
TEST(LogpMomentum, ZeroMomentum) {
  ...
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, inv_mass), 0.0);
}
...
TEST(LogpMomentum, UnitMass3Vector) {
  ...
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, inv_mass),
                   -0.5 * (1.0 + 4.0 + 9.0));
}
...
TEST(LogpMomentum, KnownValueLinearTransform) {
  ...
  rho << 2.0, 3.0;
  inv_mass << 0.5, 2.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, inv_mass), -10.0);
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, inv_mass),
                   walnutpie::detail::logp_momentum(-rho, inv_mass));
  EXPECT_DOUBLE_EQ(walnutpie::detail::logp_momentum(rho, 2.5 * inv_mass),
                   2.5 * walnutpie::detail::logp_momentum(rho, inv_mass));
}
...
TEST(NoExceptLogpGrad, NormalEvaluation) {
  GoodLogpGrad f;
  TattleTaleHandler h;
  walnutpie::detail::NoExceptLogpGrad wrapped(f, h);
  Eigen::VectorXd x(3);
  x << 1.0, 2.0, 3.0;
  double logp;
  Eigen::VectorXd grad;
...
TEST(DetailGrad, ReturnsCorrectGradient) {
  ...
  theta << 1.0, 2.0, 3.0;
  Eigen::VectorXd g = walnutpie::detail::grad(f, theta);
...
TEST(DetailL2RelDiff, SingleElement) {
  ...
  a << 2.0;
  b << 1.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::l2_rel_diff(a, b), 1.0);  // (2-1)/1 = 1
}

TEST(DetailL2RelDiff, ZeroDifference) {
  Eigen::VectorXd a(3);
  a << 1.0, 2.0, 3.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::l2_rel_diff(a, a), 0.0);
}
...
TEST(DetailL2RelDiff, KnownValue) {
  ...
  a << 3.0, 5.0;
  b << 1.0, 4.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::l2_rel_diff(a, b),
...
TEST(DetailL2RelDiff, ScaleInvariant) {
  ...
  a << 2.0, 3.0, 4.0;
  b << 1.0, 2.0, 3.0;
  EXPECT_DOUBLE_EQ(walnutpie::detail::l2_rel_diff(3.0 * a, 3.0 * b),
                   walnutpie::detail::l2_rel_diff(a, b));
}

// sum() function ***************************************************

TEST(DetailSum, KnownValues) {
  EXPECT_EQ(walnutpie::detail::sum(std::vector<std::size_t>{}), 0UL);
  EXPECT_EQ(walnutpie::detail::sum(std::vector<std::size_t>{7}), 7UL);
  EXPECT_EQ(walnutpie::detail::sum(std::vector<std::size_t>{0, 1, 2, 3, 4, 5}),
            15UL);
}
...
TEST(DetailVariance, KnownValue) {
  ...
  EXPECT_DOUBLE_EQ(walnutpie::detail::variance(xs), sq_diffs / 3.0);
}

TEST(DetailVariance, ConstantVector) {
  Eigen::VectorXd xs = Eigen::VectorXd::Constant(5, 3.7);
  EXPECT_DOUBLE_EQ(walnutpie::detail::variance(xs), 0.0);
}
...
TEST(DetailVariance, ShiftInvariantQuadraticScale) {
  ...
  xs << 1.0, 2.0, 3.0, 4.0;
  EXPECT_DOUBLE_EQ(
      walnutpie::detail::variance(xs + Eigen::VectorXd::Constant(4, 100.0)),
      walnutpie::detail::variance(xs));
  EXPECT_DOUBLE_EQ(walnutpie::detail::variance(2.0 * xs),
                   4.0 * walnutpie::detail::variance(xs));
  EXPECT_DOUBLE_EQ(
      walnutpie::detail::variance((5.7 + (13.2 * xs).array()).matrix()),
      13.2 * 13.2 * walnutpie::detail::variance(xs));
}
...
TEST(DetailLeapfrogError, ZeroStateIsZero) {
  ...
  EXPECT_DOUBLE_EQ(walnutpie::detail::leapfrog_error(f, theta, rho, inv_M, 1.0),
                   0.0);
}
...
TEST(DetailLeapfrogError, ZeroThetaUnitEverything) {
  ...
  inv_M << 0.3;
  double step = 0.75;
  EXPECT_NEAR(walnutpie::detail::leapfrog_error(f, theta, rho, inv_M, step),
...
TEST(DetailLeapfrogError, ZeroThetaTwoDimSums) {
  ...
  EXPECT_NEAR(walnutpie::detail::leapfrog_error(f, theta, rho, inv_M, 1.0),
