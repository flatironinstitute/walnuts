...
namespace walnuts {
  ...
  public:
    ...
    Eigen::VectorXd draw() const noexcept {
      return theta_;
    }

    double& logp(std::size_t n) noexcept { return logp_[static_cast<int64_t>(n)]; }

    const double& logp(std::size_t n) const noexcept { return logp_[static_cast<int64_t>(n)]; }
  ...
  static void controller_loop(std::vector<walnuts::Padded<AtomicChainStats>>& stats_by_chain,
	...
	chain_variances[static_cast<int64_t>(m)] = static_cast<double>(u.sample_var);
	counts[m] = u.count;
      }
