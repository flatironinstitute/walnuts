...
void rw1_logp_grad(const Eigen::VectorXd& y,
...
static void summarize(const Eigen::MatrixXd& draws) {
  Eigen::Index N = draws.cols();
  Eigen::Index D = draws.rows();
  for (Eigen::Index d = 0; d < D; ++d) {
    if (d > 3 && d < D - 3) {
      if (d == 4) {
        std::cout << "... elided " << (D - 6) << " rows ...\n";
      }
    }
...
static void run_nuts(const F& target_logp_grad,
  ...
  nuts::Nuts sample(rand, target_logp_grad, theta_init, inv_mass, step_size,
                    max_depth);
  Eigen::MatrixXd draws(static_cast<Eigen::Index>(D), static_cast<Eigen::Index>(N));
...
int main() {
  ...
  Eigen::VectorXd inv_mass = Eigen::VectorXd::Ones(static_cast<Eigen::Index>(D));
